          ...
          {TESTIMONIALS.map((t, idx) => (
            <Card key={idx} className="flex flex-col justify-between space-y-6">
              ...
              <div className="pt-4 border-t border-slate-800/60">
                <span className="font-bold text-slate-100 block text-sm">{t.author}</span>
                <span className="text-xs text-blue-400 font-semibold block">{t.title}</span>
                <span className="text-xs text-slate-500 block">{t.company}</span>
              </div>
            </Card>
          ))}
        </div>
