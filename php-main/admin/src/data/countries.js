// Complete World Countries Dataset for Admin Portal
...
export const DEFAULT_COUNTRY = COUNTRIES.find(c => c.code === 'AE') || COUNTRIES[0];
...
export function detectCountryFromPhone(input) {
  ...
  // 1. Check with leading + (e.g. +971501234567, +919876543210)
  ...
  // 2. Check with leading 00 (e.g. 00971501234567, 0096651234567)
  if (cleaned.startsWith('00')) {
    const without00 = '+' + cleaned.slice(2);
    for (const country of sortedCountries) {
      if (without00.startsWith(country.dialCode)) {
        return {
          country,
          dialCode: country.dialCode,
          localNumber: without00.slice(country.dialCode.length)
        };
      }
    }
  }

  // 3. Direct dial code match without + (e.g. 971501234567, 919876543210)
  if (cleaned.length >= 8) {
    for (const country of sortedCountries) {
      ...
      if (cleaned.startsWith(codeWithoutPlus)) {
        return {
          country,
          dialCode: country.dialCode,
          localNumber: cleaned.slice(codeWithoutPlus.length)
        };
      }
    }
  }
