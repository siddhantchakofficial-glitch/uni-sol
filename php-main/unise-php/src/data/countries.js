// Complete World Countries Dataset with Flags and Dial Codes
...
// Sorted dial codes by length descending to match longest prefix first
const sortedByDialLength = [...COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length);

/**
 * Automatically detects country and extracts dial code and local phone number
 * Handles formats like:
 * +971501234567, +91 9876543210, 0096651234567, +1-415-555-2671, etc.
 */
export function detectCountryFromPhone(input) {
  if (!input || typeof input !== 'string') return null;
  const raw = input.trim();
  
  // Check if input begins with + or 00 or direct international prefix
  let clean = raw;
  let isExplicitInternational = false;

  if (clean.startsWith('+')) {
    clean = clean.substring(1).replace(/[\s\-\(\)]/g, '');
    isExplicitInternational = true;
  } else if (clean.startsWith('00')) {
    clean = clean.substring(2).replace(/[\s\-\(\)]/g, '');
    isExplicitInternational = true;
  } else {
  ...
  for (const item of sortedByDialLength) {
    const rawDial = item.dialCode.replace('+', '');
    if (clean.startsWith(rawDial)) {
      const remainder = clean.substring(rawDial.length);
      // Only match if explicit (+ / 00) OR if enough digits exist (e.g. 7+ digits total)
      if (isExplicitInternational || remainder.length >= 6) {
        return {
          country: item,
          dialCode: item.dialCode,
          localNumber: remainder.trim()
        };
      }
    }
  }
